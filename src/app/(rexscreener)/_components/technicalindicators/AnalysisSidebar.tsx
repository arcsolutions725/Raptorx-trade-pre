...
export default function AnalysisSidebar({
  ...
  const formatAnalysisText = useCallback((text: string) => {
    return text.split("\n").map((line, i) => {
      ...
      if (line.startsWith("## ")) {
        return (
          <h2 key={i} className="text-lg font-bold text-[#ffc000] mt-4 mb-2">
            {line.substring(3)}
          </h2>
        );
      }
      ...
      if (line.startsWith("### ")) {
        return (
          <h3 key={i} className="text-md font-semibold text-[#ffc000] mt-3 mb-1">
            {line.substring(4)}
          </h3>
        );
      }
        ...
        <div className="mb-4 border border-white/10 rounded-lg overflow-hidden">
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-[#ffc000] font-semibold">{title}</h3>
            </div>
  ...
  return (
    <div className="w-full md:w-96 h-full bg-black/80 border-l border-white/10 overflow-y-auto scrollbar-thin">
      ...
      <div className="sticky top-0 z-10 flex items-center justify-between p-4 bg-black/90 border-b border-white/10">
        <h2 className="text-[#ffc000] font-semibold text-lg">Technical Analysis</h2>
        {onClose && (
          <button
            type="button"
            onClick={onClose}
