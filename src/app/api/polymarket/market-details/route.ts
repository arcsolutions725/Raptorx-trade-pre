...
export async function GET(request: NextRequest) {
  try {
    ...
    const transformedMarkets = marketsArray.map((market: any) => {
      ...
      const volume24hr =
        market.volume24hr || parseFloat(market.volume24hr || "0");
      ...
      const conditionId =
        market.conditionId || market.condition_id || market.id || "";
      ...
      const marketId =
        market.id || market.marketId || market.market_id || conditionId;
      ...
      return {
        ...
        subtitle:
          market.question || market.groupItemTitle || outcomes[0] || "Outcome",
        groupItemTitle:
          market.groupItemTitle || market.question || outcomes[0] || "Outcome",
    ...
    const seriesId =
      event.series && event.series.length > 0 ? event.series[0].id : null;
